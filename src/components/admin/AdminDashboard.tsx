...
import {
  Archive, ArrowDown, ArrowUp, BarChart3, Boxes, CheckCircle2, ClipboardList, DollarSign, Edit3,
  ImagePlus, Loader2, LogOut, PackagePlus, RefreshCw, RotateCcw, Save, ShieldCheck,
  ShoppingBag, Star, Trash2, UserPlus, Users, X,
} from 'lucide-react';
...
type AdminUser = {
  ...
  can_reset_password?: boolean; email_confirmed?: boolean; last_sign_in_at?: string | null; created_at: string;
};
type Tab = 'overview' | 'business' | 'products' | 'quotes' | 'team';
...
function ProductEditForm({ product, onClose, onSaved, onUpdated }: { product: AdminProduct; onClose: () => void; onSaved: (product: AdminProduct) => void; onUpdated: (product: AdminProduct) => void }) {
  const [currentProduct, setCurrentProduct] = useState(product);
  useEffect(() => { setCurrentProduct(product); }, [product]);
  const sortedImages = [...(currentProduct.images || [])].sort((a, b) => a.sort_order - b.sort_order);
  const [draft, setDraft] = useState({ name_es: product.name_es, name_en: product.name_en, description_es: product.description_es || '', description_en: product.description_en || '', price: Number(product.price), cost_price: product.cost_price == null ? null : Number(product.cost_price), stock: product.variants?.[0]?.stock_quantity ?? 0, material: product.material, color: product.variants?.[0]?.color || 'Estándar', size: product.variants?.[0]?.size || 'Estándar', status: product.status, is_featured: product.is_featured });
  useEffect(() => { setDraft({ name_es: product.name_es, name_en: product.name_en, description_es: product.description_es || '', description_en: product.description_en || '', price: Number(product.price), cost_price: product.cost_price == null ? null : Number(product.cost_price), stock: product.variants?.[0]?.stock_quantity ?? 0, material: product.material, color: product.variants?.[0]?.color || 'Estándar', size: product.variants?.[0]?.size || 'Estándar', status: product.status, is_featured: product.is_featured }); }, [product]);
  const [saving, setSaving] = useState(false); const [imageSaving, setImageSaving] = useState(false); const [error, setError] = useState('');
  const applyImageResult = (updated: AdminProduct) => { setCurrentProduct(updated); onUpdated(updated); };
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { const result = await api<{ product: AdminProduct }>(`/api/admin/products/${currentProduct.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(draft) }); onSaved(result.product); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No fue posible editar.'); } finally { setSaving(false); } };
  const uploadImages = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setImageSaving(true); setError('');
    try {
      const result = await api<{ product: AdminProduct }>(`/api/admin/products/${currentProduct.id}/images`, { method: 'POST', body: new FormData(event.currentTarget) });
      applyImageResult(result.product); event.currentTarget.reset();
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'No fue posible subir imágenes.'); }
    finally { setImageSaving(false); }
  };
  const updateImages = async (body: Record<string, unknown>) => { setImageSaving(true); setError(''); try { const result = await api<{ product: AdminProduct }>(`/api/admin/products/${currentProduct.id}/images`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); applyImageResult(result.product); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No fue posible actualizar imágenes.'); } finally { setImageSaving(false); } };
  const deleteImage = async (imageId: string) => { if (!confirm('¿Eliminar esta imagen del producto?')) return; setImageSaving(true); setError(''); try { const result = await api<{ product: AdminProduct }>(`/api/admin/products/${currentProduct.id}/images`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image_id: imageId }) }); applyImageResult(result.product); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No fue posible eliminar la imagen.'); } finally { setImageSaving(false); } };
  const moveImage = (imageId: string, direction: -1 | 1) => { const index = sortedImages.findIndex((image) => image.id === imageId); const target = index + direction; if (index < 0 || target < 0 || target >= sortedImages.length) return; const order = [...sortedImages]; [order[index], order[target]] = [order[target], order[index]]; void updateImages({ order: order.map((image) => image.id) }); };
  return <div className="space-y-4 rounded-2xl border border-brand-orange/30 bg-brand-dark-card p-5"><div className="mb-4 flex justify-between"><h3 className="font-heading font-bold">Editar {currentProduct.name_es}</h3><button onClick={onClose} aria-label="Cerrar edición de producto"><X className="h-5 w-5" /></button></div>
    <form aria-label="Editar producto" id="product-edit-form" onSubmit={submit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
      <label className={labelClass}>Nombre ES<input id="product-edit-name_es" name="name_es" value={draft.name_es} onChange={(e) => setDraft({ ...draft, name_es: e.target.value })} className={inputClass} /></label><label className={labelClass}>Nombre EN<input id="product-edit-name_en" name="name_en" value={draft.name_en} onChange={(e) => setDraft({ ...draft, name_en: e.target.value })} className={inputClass} /></label>
      <label className={labelClass}>Precio<input id="product-edit-price" name="price" type="number" min="0.01" step="0.01" value={draft.price} onChange={(e) => setDraft({ ...draft, price: Number(e.target.value) })} className={inputClass} /></label><label className={labelClass}>Costo<input id="product-edit-cost_price" name="cost_price" type="number" min="0" step="0.01" value={draft.cost_price ?? ''} onChange={(e) => setDraft({ ...draft, cost_price: e.target.value === '' ? null : Number(e.target.value) })} className={inputClass} /></label>
      <label className={labelClass}>Inventario<input id="product-edit-stock" name="stock" type="number" min="0" value={draft.stock} onChange={(e) => setDraft({ ...draft, stock: Number(e.target.value) })} className={inputClass} /></label><label className={labelClass}>Estado<select id="product-edit-status" name="status" value={draft.status} onChange={(e) => setDraft({ ...draft, status: e.target.value as ProductStatus })} className={inputClass}>{productStatuses.map((status) => <option key={status}>{status}</option>)}</select></label>
      <label className={labelClass}>Material<input id="product-edit-material" name="material" value={draft.material} onChange={(e) => setDraft({ ...draft, material: e.target.value })} className={inputClass} /></label><label className={labelClass}>Color<input id="product-edit-color" name="color" value={draft.color} onChange={(e) => setDraft({ ...draft, color: e.target.value })} className={inputClass} /></label>
      <label className={`${labelClass} md:col-span-2`}>Descripción ES<textarea id="product-edit-description_es" name="description_es" rows={3} value={draft.description_es} onChange={(e) => setDraft({ ...draft, description_es: e.target.value })} className={inputClass} /></label><label className={`${labelClass} md:col-span-2`}>Descripción EN<textarea id="product-edit-description_en" name="description_en" rows={3} value={draft.description_en} onChange={(e) => setDraft({ ...draft, description_en: e.target.value })} className={inputClass} /></label>
      <label className="flex items-center gap-2 text-xs"><input id="product-edit-is_featured" name="is_featured" type="checkbox" checked={draft.is_featured} onChange={(e) => setDraft({ ...draft, is_featured: e.target.checked })} />Producto destacado</label>
      <div className="md:col-span-2 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-xl border px-4 py-2 text-xs">Cancelar</button><button disabled={saving} className="flex items-center gap-2 rounded-xl bg-brand-orange px-4 py-2 text-xs font-bold text-slate-950"><Save className="h-4 w-4" />Guardar datos</button></div>
    </form>
    <section className="space-y-3 rounded-2xl border border-brand-dark-border bg-brand-dark p-4">
      <div><h4 className="font-heading text-sm font-bold text-slate-100">Imágenes del producto</h4><p className="text-xs text-slate-400">Sube hasta 8 fotos por producto, marca la principal y reordénalas para la ficha pública.</p></div>
      <form aria-label="Subir imágenes del producto" id="product-images-upload-form" onSubmit={uploadImages} className="flex flex-col gap-3 md:flex-row md:items-end"><label className={`${labelClass} flex-1`}>Añadir imágenes<input id="product-images-upload" name="images" type="file" accept="image/png,image/jpeg,image/webp" multiple className={inputClass} /></label><button disabled={imageSaving} className="flex items-center justify-center gap-2 rounded-xl bg-brand-cyan px-4 py-2.5 text-xs font-bold text-slate-950"><ImagePlus className="h-4 w-4" />Subir fotos</button></form>
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{sortedImages.map((image, index) => <div key={image.id} className="rounded-xl border border-brand-dark-border bg-brand-dark-card p-3"><img src={image.url} alt={image.alt_text || currentProduct.name_es} className="mb-3 h-32 w-full rounded-lg object-cover" /><div className="flex flex-wrap gap-2"><button type="button" disabled={imageSaving || image.is_primary} onClick={() => void updateImages({ primary_image_id: image.id })} className="flex items-center gap-1 rounded-lg border px-2 py-1 text-[11px] disabled:opacity-50"><Star className="h-3 w-3" />{image.is_primary ? 'Principal' : 'Hacer principal'}</button><button type="button" disabled={imageSaving || index === 0} onClick={() => moveImage(image.id, -1)} aria-label="Mover imagen arriba" className="rounded-lg border px-2 py-1 disabled:opacity-50"><ArrowUp className="h-3 w-3" /></button><button type="button" disabled={imageSaving || index === sortedImages.length - 1} onClick={() => moveImage(image.id, 1)} aria-label="Mover imagen abajo" className="rounded-lg border px-2 py-1 disabled:opacity-50"><ArrowDown className="h-3 w-3" /></button><button type="button" disabled={imageSaving || sortedImages.length <= 1} onClick={() => void deleteImage(image.id)} className="rounded-lg border border-red-800 px-2 py-1 text-red-300 disabled:opacity-50"><Trash2 className="h-3 w-3" /></button></div></div>)}</div>
    </section>
    {error && <p className="rounded-xl border border-red-800 bg-red-950/40 p-3 text-xs text-red-300">{error}</p>}
  </div>;
}
...
export function AdminDashboard({ lang }: { lang: string }) {
  ...
  const activeProducts = useMemo(() => products.filter((product) => !product.archived_at), [products]);
  const totalInventory = useMemo(() => products.reduce((sum, product) => sum + (product.variants?.reduce((n, variant) => n + variant.stock_quantity, 0) || 0), 0), [products]);
  const lowInventoryProducts = useMemo(() => activeProducts.filter((product) => (product.variants?.reduce((sum, variant) => sum + variant.stock_quantity, 0) || 0) <= 3), [activeProducts]);
  const madeToOrderCount = useMemo(() => activeProducts.filter((product) => product.status === 'MADE_TO_ORDER').length, [activeProducts]);
  const readyToShipCount = useMemo(() => activeProducts.filter((product) => product.status === 'AVAILABLE' || product.status === 'READY_TO_SHIP').length, [activeProducts]);
  const activeQuoteCount = useMemo(() => quotes.filter((quote) => !['COMPLETED', 'CANCELLED', 'REJECTED'].includes(quote.status)).length, [quotes]);
  const quotedValue = useMemo(() => quotes.reduce((sum, quote) => sum + Number(quote.budget || 0), 0), [quotes]);
  const averageOrderValue = Number(kpis?.ordersActive || 0) > 0 ? Number(kpis?.salesMonth || 0) / Number(kpis?.ordersActive || 1) : 0;
  ...
  return <div className="container mx-auto space-y-6 px-4 py-8">
    ...
    {(error || notice) && <div className={`flex items-center justify-between rounded-xl border p-3 text-xs ${error ? 'border-red-800 bg-red-950/30 text-red-300' : 'border-green-800 bg-green-950/30 text-green-300'}`}><span>{error || notice}</span><button aria-label="Cerrar aviso" onClick={() => { setError(''); setNotice(''); }}><X className="h-4 w-4" /></button></div>}
    <nav className="flex gap-2 overflow-x-auto border-b border-brand-dark-border pb-2">{([['overview','Resumen'], ['business','Negocio'], ...(can('manage_products') ? [['products','Productos']] : []), ...(can('manage_quotes') ? [['quotes','Cotizaciones']] : []), ...(can('manage_admins') ? [['team','Administradores']] : [])] as [Tab,string][]).map(([id,label]) => <button key={id} onClick={() => setActiveTab(id)} className={`rounded-xl px-4 py-2 text-xs font-bold ${activeTab === id ? 'bg-brand-cyan text-slate-950' : 'text-slate-400 hover:bg-brand-dark-card'}`}>{label}</button>)}</nav>

    {activeTab === 'overview' && <section className="space-y-5"><div className="grid grid-cols-2 gap-4 lg:grid-cols-4"><Kpi label="Ventas del mes" value={`$${Number(kpis?.salesMonth || 0).toFixed(2)}`} icon={DollarSign} /><Kpi label="Órdenes activas" value={String(kpis?.ordersActive || 0)} icon={ShoppingBag} /><Kpi label="Productos disponibles" value={String(availableCount)} icon={Boxes} /><Kpi label="Cotizaciones pendientes" value={String(kpis?.customQuotesPending || 0)} icon={ClipboardList} /></div><div className="rounded-2xl border border-brand-dark-border bg-brand-dark-card p-5"><h2 className="mb-4 flex items-center gap-2 font-heading font-bold"><BarChart3 className="h-5 w-5 text-brand-cyan" />Estado operativo</h2><div className="grid gap-3 text-sm md:grid-cols-3"><p>Inventario administrable: <b>{totalInventory} unidades</b></p><p>Cotizaciones registradas: <b>{quotes.length}</b></p><p>Administradores activos: <b>{team.filter((person) => person.active).length || (admin ? 1 : 0)}</b></p></div></div></section>}


    {activeTab === 'business' && <section className="space-y-5">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4"><Kpi label="Ventas del mes" value={`$${Number(kpis?.salesMonth || 0).toFixed(2)}`} icon={DollarSign} /><Kpi label="Ticket promedio estimado" value={`$${averageOrderValue.toFixed(2)}`} icon={ShoppingBag} /><Kpi label="Valor cotizado" value={`$${quotedValue.toFixed(2)}`} icon={ClipboardList} /><Kpi label="Inventario total" value={String(totalInventory)} icon={Boxes} /></div>
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl border border-brand-dark-border bg-brand-dark-card p-5"><h2 className="mb-3 flex items-center gap-2 font-heading font-bold"><BarChart3 className="h-5 w-5 text-brand-cyan" />Resumen comercial</h2><div className="space-y-2 text-sm text-slate-300"><p>Productos publicados: <b>{availableCount}</b></p><p>Listos / disponibles: <b>{readyToShipCount}</b></p><p>Bajo pedido: <b>{madeToOrderCount}</b></p><p>Archivados: <b>{products.length - availableCount}</b></p></div></div>
        <div className="rounded-2xl border border-brand-dark-border bg-brand-dark-card p-5"><h2 className="mb-3 flex items-center gap-2 font-heading font-bold"><ClipboardList className="h-5 w-5 text-brand-orange" />Operación</h2><div className="space-y-2 text-sm text-slate-300"><p>Órdenes activas: <b>{kpis?.ordersActive || 0}</b></p><p>Cotizaciones activas: <b>{activeQuoteCount}</b></p><p>Cotizaciones pendientes: <b>{kpis?.customQuotesPending || 0}</b></p><p>Administradores activos: <b>{team.filter((person) => person.active).length || (admin ? 1 : 0)}</b></p></div></div>
        <div className="rounded-2xl border border-brand-dark-border bg-brand-dark-card p-5"><h2 className="mb-3 flex items-center gap-2 font-heading font-bold"><Boxes className="h-5 w-5 text-brand-cyan" />Alertas de catálogo</h2>{lowInventoryProducts.length === 0 ? <p className="text-sm text-green-300">Sin productos con inventario bajo.</p> : <div className="space-y-2 text-sm text-slate-300">{lowInventoryProducts.slice(0, 6).map((product) => <p key={product.id} className="flex justify-between gap-3"><span>{product.name_es}</span><b>{product.variants?.reduce((sum, variant) => sum + variant.stock_quantity, 0) || 0}</b></p>)}</div>}</div>
      </div>
      <div className="rounded-2xl border border-brand-dark-border bg-brand-dark-card p-5"><h2 className="mb-3 flex items-center gap-2 font-heading font-bold"><CheckCircle2 className="h-5 w-5 text-green-400" />Próximas acciones recomendadas</h2><ul className="grid gap-2 text-sm text-slate-300 md:grid-cols-2"><li>• Revisar precios provisionales de miniaturas.</li><li>• Confirmar stock real vs. bajo pedido.</li><li>• Atender cotizaciones activas y presupuestos sin monto.</li><li>• Mantener Stripe en test hasta configurar live.</li></ul></div>
    </section>}

    {activeTab === 'products' && can('manage_products') && <section className="space-y-4">{showCreate && <ProductCreateForm onClose={() => setShowCreate(false)} onCreated={(product) => { setProducts((current) => [product, ...current]); setShowCreate(false); setNotice('Producto creado y publicado.'); }} />}{editing && <ProductEditForm product={editing} onClose={() => setEditing(null)} onUpdated={(product) => { replaceProduct(product); setEditing(product); setNotice('Imágenes del producto actualizadas.'); }} onSaved={(product) => { replaceProduct(product); setEditing(null); setNotice('Producto actualizado.'); }} />}<div className="rounded-3xl border border-brand-dark-border bg-brand-dark-card p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-heading text-lg font-bold">Catálogo actual</h2><p className="text-xs text-slate-400">{availableCount} publicados · {products.length - availableCount} archivados</p></div><button onClick={() => { setShowCreate(true); setEditing(null); }} className="flex items-center gap-2 rounded-xl bg-brand-cyan px-4 py-2 text-xs font-bold text-slate-950"><PackagePlus className="h-4 w-4" />Añadir producto</button></div><div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left text-xs"><thead className="border-b border-brand-dark-border text-slate-400"><tr><th className="p-3">Producto</th><th>SKU</th><th>Precio</th><th>Stock</th><th>Estado</th><th>Visibilidad</th><th className="text-right">Acciones</th></tr></thead><tbody>{products.map((product) => <tr key={product.id} className="border-b border-brand-dark-border/60"><td className="p-3 font-bold">{product.name_es}</td><td className="font-mono text-brand-cyan">{product.sku}</td><td>${Number(product.price).toFixed(2)}</td><td>{product.variants?.reduce((sum, variant) => sum + variant.stock_quantity, 0) || 0}</td><td>{product.status}</td><td>{product.archived_at ? <span className="text-red-400">Archivado</span> : <span className="text-green-400">Publicado</span>}</td><td><div className="flex justify-end gap-2"><button onClick={() => { setEditing(product); setShowCreate(false); }} className="rounded-lg p-2 text-brand-cyan" title="Editar" aria-label={`Editar ${product.name_es}`}><Edit3 className="h-4 w-4" /></button>{product.archived_at ? <button onClick={() => void restoreProduct(product)} className="rounded-lg p-2 text-green-400" title="Restaurar" aria-label={`Restaurar ${product.name_es}`}><RotateCcw className="h-4 w-4" /></button> : <button onClick={() => void archiveProduct(product)} className="rounded-lg p-2 text-red-400" title="Quitar" aria-label={`Archivar ${product.name_es}`}><Archive className="h-4 w-4" /></button>}</div></td></tr>)}</tbody></table></div></div></section>}
...
function QuoteEditor({ quote, onSaved }: { quote: AdminQuote; onSaved: (quote: AdminQuote) => void }) {
  ...
  return <div className="space-y-4 rounded-2xl border border-brand-dark-border bg-brand-dark-card p-5"><div><h2 className="font-heading text-lg font-bold">{quote.project_name}</h2><p className="text-xs text-slate-400">{quote.request_number} · {new Date(quote.created_at).toLocaleString('es-PR')}</p></div><div className="grid gap-3 text-sm md:grid-cols-2"><p><b>Cliente:</b> {quote.customer_name}</p><p><b>Email:</b> {quote.customer_email}</p><p><b>Teléfono:</b> {quote.customer_phone || '—'}</p><p><b>Cantidad:</b> {quote.quantity}</p><p><b>Material:</b> {quote.material || '—'}</p><p><b>Tamaño:</b> {quote.desired_size || '—'}</p></div><div className="rounded-xl bg-brand-dark p-4 text-sm whitespace-pre-wrap">{quote.description}</div><label className={labelClass}>Estado<select id="quote-edit-status" name="status" value={status} onChange={(e) => setStatus(e.target.value as CustomRequestStatus)} className={inputClass}>{quoteStatuses.map((value) => <option key={value}>{value}</option>)}</select></label><label className={labelClass}>Presupuesto / cotización estimada<input id="quote-edit-budget" name="budget" type="number" min="0" step="0.01" value={budget} onChange={(e) => setBudget(e.target.value)} className={inputClass} /></label><label className={labelClass}>Notas internas<textarea id="quote-edit-admin_notes" name="admin_notes" rows={5} value={notes} onChange={(e) => setNotes(e.target.value)} className={inputClass} /></label>{error && <p className="text-xs text-red-400">{error}</p>}<button onClick={() => void save()} disabled={saving} className="flex items-center gap-2 rounded-xl bg-brand-cyan px-4 py-2 text-xs font-bold text-slate-950"><Save className="h-4 w-4" />Guardar cotización</button></div>;
}
...
function TeamManager({ currentAdmin, team, setTeam, setNotice, setError }: { currentAdmin: AdminIdentity; team: AdminUser[]; setTeam: React.Dispatch<React.SetStateAction<AdminUser[]>>; setNotice: (value: string) => void; setError: (value: string) => void }) {
  ...
  const resetPassword = async (person: AdminUser) => {
    if (!confirm(`¿Enviar un enlace seguro de restablecimiento a ${person.email}?`)) return;
    setResetting(person.id); setError('');
    try {
      await api(`/api/admin/users/${person.id}/reset-password`, { method: 'POST' });
      setNotice('Enlace enviado por correo. Solo el destinatario puede elegir su contraseña.');
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'No fue posible enviar el enlace.'); }
  ...
  return <section className="space-y-4">{adding && <div className="rounded-2xl border border-brand-cyan/30 bg-brand-dark-card p-5"><form aria-label="Invitar administrador" id="admin-invite-form" onSubmit={add} className="grid gap-4 md:grid-cols-3"><label className={labelClass}>Nombre completo<input id="admin-invite-full_name" name="full_name" required className={inputClass} /></label><label className={labelClass}>Correo<input id="admin-invite-email" name="email" type="email" required className={inputClass} /></label><label className={labelClass}>Rol<select id="admin-invite-role" name="role" className={inputClass}>{adminRoles.map((role) => <option key={role}>{role}</option>)}</select></label><div className="md:col-span-3 flex justify-end gap-2"><button type="button" onClick={() => setAdding(false)} className="rounded-xl border px-4 py-2 text-xs">Cancelar</button><button disabled={saving} className="flex items-center gap-2 rounded-xl bg-brand-cyan px-4 py-2 text-xs font-bold text-slate-950"><UserPlus className="h-4 w-4" />Añadir e invitar</button></div></form></div>}<div className="rounded-3xl border border-brand-dark-border bg-brand-dark-card p-5"><div className="mb-4 flex justify-between"><div><h2 className="flex items-center gap-2 font-heading text-lg font-bold"><Users className="h-5 w-5 text-brand-cyan" />Equipo administrativo</h2><p className="text-xs text-slate-400">Solo el propietario puede invitar, gestionar acceso o enviar enlaces de restablecimiento.</p></div><button onClick={() => setAdding(true)} className="flex items-center gap-2 rounded-xl bg-brand-cyan px-4 py-2 text-xs font-bold text-slate-950"><UserPlus className="h-4 w-4" />Agregar persona</button></div><div className="space-y-3">{team.map((person) => <div key={person.id} className="grid items-center gap-3 rounded-2xl border border-brand-dark-border p-4 md:grid-cols-[1fr_220px_120px]"><div><div>{currentAdmin.permissions.includes('manage_admins') && person.can_reset_password && person.active && person.id !== currentAdmin.id && <button type="button" disabled={resetting !== null} onClick={() => void resetPassword(person)} className="mb-2 rounded border px-3 py-2 text-xs">{resetting === person.id ? 'Enviando…' : 'Restablecer por email'}</button>}</div><p className="font-bold">{person.full_name} {person.id === currentAdmin.id && <span className="text-xs text-brand-cyan">(Tú)</span>}</p><p className="text-xs text-slate-400">{person.email}</p><p className="text-[10px] text-slate-500">{person.email_confirmed ? 'Correo confirmado' : person.invited ? 'Invitación pendiente' : 'Sin cuenta Auth detectada'}</p></div><select id={`admin-role-${person.id}`} name={`admin-role-${person.id}`} aria-label={`Rol de ${person.email}`} disabled={person.id === currentAdmin.id || !person.active} value={person.role} onChange={(e) => void update(person, { role: e.target.value as AdminRole })} className={inputClass}>{adminRoles.map((role) => <option key={role}>{role}</option>)}</select><div className="flex justify-end">{person.active ? <button aria-label={`Revocar acceso de ${person.email}`} disabled={person.id === currentAdmin.id} onClick={() => { if (confirm(`¿Revocar el acceso de ${person.full_name}?`)) void update(person, { active: false }); }} className="flex items-center gap-1 text-xs text-red-400 disabled:opacity-30"><Trash2 className="h-4 w-4" />Revocar</button> : <button aria-label={`Reactivar acceso de ${person.email}`} onClick={() => void update(person, { active: true })} className="flex items-center gap-1 text-xs text-green-400"><CheckCircle2 className="h-4 w-4" />Reactivar</button>}</div></div>)}</div></div></section>;
}
